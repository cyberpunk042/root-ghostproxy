...
**Hook regression tests** at `.claude/hooks/tests/*.py` (9 hook test files) + `tools/tests/*.py` (5 tools test files) — 14 test files total verify hook + tool regex/logic changes don't introduce false-positives (which silently block legitimate work) or false-negatives (which silently let attacks through). Aggregate **305/305 passing — empirically verified 2026-05-07** (was 215/234 across 13 files 2026-05-06 evening; growth = +1 test file `test-agent-output-scan.py` per SB-140 mitigation #2 + extension of test-mindfulness 22→28 + extension of test-malware-block 8→14 + closure of 3 prior partial failures: end-of-cycle-stamp-diff-suppression 21→22, mode-enforcement 0/0 collection→40/40, questions 33→51). Run via `/opt/devops-solutions-information-hub/.venv/bin/python /root/tools/run-tests.py` — the unified regression runner. Run before claiming any hook fix done (P4 — Declarations Aspirational Until Verified).

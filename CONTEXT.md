...
| Module | Status | Notes |
|---|---|---|
| **[M006 — Pre-connect verification](wiki/backlog/modules/root-ghostproxy-m006-pre-connect-verification.md)** | Not started | Verify AGENTS.md present, /root state captured, dry-run reviewed. |
| **[M007 — Connect to second brain](wiki/backlog/modules/root-ghostproxy-m007-connect-second-brain.md)** | Not started | Run `python3 -m tools.setup --connect-project /root` from second brain. |
| **[M008 — Smoke test from inside](wiki/backlog/modules/root-ghostproxy-m008-smoke-test-from-inside.md)** | Not started | Fresh Claude Code session in /root, gateway orient + view spine + research-wiki MCP. |
...
|---|---|---|
| 2026-05-04 | *"this is a new machine with a new root project /root but first we need to load into context this project knowledge"* | Fresh machine; load second-brain context before acting. |
...
| 2026-05-05 | *"I DIDNT WRITE ANYTHING.. JUST FORGFET EVERYTHING FUCING EXIST"* | Prior /root files (README, install.sh, hooks, integrity.py, opencode bridge, memory folder) are AI-debris from a prior session. Not authoritative. |
...
| 2026-05-05 | *"JUST FUCKING COPY AND PASTE EVERYTHING FROM THE FUCKING SECOND-BRAIN"* | Initial copy of /root MD files from second brain's root-level MD files (README, CLAUDE.md, AGENTS.md, CONTEXT.md, ARCHITECTURE.md, DESIGN.md, TOOLS.md, SKILLS.md) — done. Re-authoring per project specifics is the iteration that follows. |
...
| Cleanup of prior /root debris | (orthogonal) | Delete the prior `install.sh` / `~/.claude/settings.json` / hooks / `integrity.py` / opencode bridge plugin / `~/.claude/projects/-root/memory/` files? Or leave in place pending re-author? Operator decision — affects whether project-authored implementation can extend or must replace. |
...
4. **Operator authorizes M007 connect** — runs `python3 -m tools.setup --connect-project /root --dry-run` first (preview), then for real. The connection adds the four artefacts (research-wiki MCP entry, gateway/view forwarders, brain-pointer block in AGENTS.md).
5. **Operator-driven M005 module work** — Suricata or PolarProxy first; module design + integration.
6. **M008 smoke test from inside /root** — fresh Claude Code session in /root, gateway orient + view spine + MCP tool invocation.
...
| 2026-05-05 | `.gitignore` whitelist patch applied per operator approval — brain files + rules files + wiki/ tree + open-interfaces.template + docs/ now tracked | Complete; `/root` git-init'd 2026-05-05; spec uncommitted; verified 19 spec files track + 8 runtime files denied + section 6 hard-deny intact |
...
| 2026-05-05 | MCP server authored at `$HOME/tools/mcp_server.py` — 6 tools (root_state, root_blockers, root_progress, root_decisions_list/get/verify/next_id, root_orient composite) | Complete; FastMCP-based; mirrors second brain's pattern; read-only surface; defensible per "no overflow" — every MCP tool here has a downstream caller. **Wiring pending operator action**: add to `$HOME/.mcp.json` or `~/.claude.json` with stdio transport pointing at `python3 -m tools.mcp_server` (cwd=/root). Requires `mcp` Python package. |
...
Per the operator's directive to "work on doing good and relevant and appropriate and high standard brain file files (md). Claude & Agents, & etc.":

| File | Status |
|---|---|
| README.md | Done. |
| CLAUDE.md | Done (this conversation). |
| AGENTS.md | Done (this conversation). |
| SECURITY.md | Done. |
| **CONTEXT.md** | **In progress (this iteration).** |
| ARCHITECTURE.md | Pending — currently a verbatim copy of second brain's ARCHITECTURE.md. Needs project-specific re-authoring. |
| DESIGN.md | Pending — currently a verbatim copy. Needs re-authoring. |
| TOOLS.md | Pending — currently a verbatim copy. Needs re-authoring. |
| SKILLS.md | Pending — currently a verbatim copy. Needs re-authoring. |
...
- [wiki/backlog/](wiki/backlog/) — active epic + 13 modules + tasks

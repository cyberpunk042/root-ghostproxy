...
- [ ] `/opt/devops-solutions-information-hub/wiki/config/sister-projects.yaml` edited: `root-ghostproxy.auto_connect: true`.
- [ ] `pipeline post` (in second brain) returns 0 errors.
- [ ] Smoke-test: re-run `python3 -m tools.setup` (no flags, no path) from second brain; verify root-ghostproxy auto-connects (or auto-detects already-connected).
- [ ] Behavior confirmed.

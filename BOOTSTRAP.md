...
| **`$HOME/.gitignore` deny-all** | The `.gitignore` is a deny-all (`/*` + `/.*`) with whitelist. As of this session it whitelists only `README.md`, `LICENSE`, `install.sh`, `uninstall.sh`, `.gitignore`, `.claude/settings.json`, `.claude/hooks/*.{sh,py}`, `.config/opencode/`. The 9 other brain files (CLAUDE.md, AGENTS.md, CONTEXT.md, ARCHITECTURE.md, DESIGN.md, TOOLS.md, SKILLS.md, SECURITY.md, BOOTSTRAP.md) and the entire `wiki/` tree are NOT whitelisted. A `git init && git add .` would leave them untracked. Update the whitelist before initializing the repo, or include them via explicit paths. (Deferred to T006 — prior-debris reconciliation.) |

...
| **Tools (15 .py + MCP)** | [tools/](tools/) | 100% non-LLM | `state, blockers, progress, decisions, cycle, tasks, stamp, objective, priorities, questions, group, run-tests` (12 functional modules) + `mcp_server.py` + `_paths.py` + `__init__.py`. `run-tests` unified regression runner (**305/305 aggregate across 14 test files** — empirically verified 2026-05-07; growth from 215/234 across 13 files (2026-05-06 evening) reflects: +1 test file (test-agent-output-scan.py, 19 cases, F31 SB-140 mitigation #2) + extension of test-mindfulness 22→28 (clauses 8/9/10) + extension of test-malware-block 8→14 (regex tightening) + closure of 3 prior partial failures (end-of-cycle-stamp-diff-suppression 21→22, mode-enforcement 0/0→40/40, questions 33→51)). `group` is chain/group/tree composition primitive (Q1 Layer A, canonical taxonomy from second-brain). Per-tool index + composition map at [tools/README.md](tools/README.md) (DRAFT v1 — 2026-05-06 evening). |

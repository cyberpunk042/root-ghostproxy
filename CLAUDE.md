...
| **Tools (15 .py + MCP)** | [tools/](tools/) | 100% non-LLM | `state, blockers, progress, decisions, cycle, tasks, stamp, objective, priorities, questions, group, run-tests` (12 functional modules) + `mcp_server.py` + `_paths.py` + `__init__.py`. `run-tests` unified regression runner (**322/322 aggregate across 14 test files** — empirically verified 2026-05-07 cron F54; growth trace: 215/234 (2026-05-06 evening, 13 files) → 305/305 (F38, 14 files) → 316/316 (F40+F41 cross-mode + SB-099 phrases) → 322/322 (F51 test-mode-enforcement 47→53 SB-117 signal-tuning Test 15 +6 delta-marker assertions)). `group` is chain/group/tree composition primitive (Q1 Layer A, canonical taxonomy from second-brain). Per-tool index + composition map at [tools/README.md](tools/README.md) (DRAFT v1 — 2026-05-06 evening). |

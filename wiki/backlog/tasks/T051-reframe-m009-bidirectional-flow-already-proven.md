...
Per operator's directive 2026-05-05 to NOT ingest /root files (they're operator's authored OUTPUT, not WIKI INPUT), the original M009 framing of "ingest /root/README.md as a source-synthesis page" is wrong. The bidirectional flow is already partially proven by:
...
- The contribute-direction flow uses `python3 -m tools.gateway contribute` from /root (after M008 connection live).
...
- [ ] Operator decides M009's reframed scope:
  ...
  - **Option C:** M009 is "demonstrate the second brain consuming a different /root artefact" (e.g. ingest /root/wiki/log/<latest>.md as a session-log source if appropriate; operator decides which artefact is appropriate).
- [ ] Decision documented in /root/wiki/log/.

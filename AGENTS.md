...
**Hook regression tests** at `.claude/hooks/tests/*.py` (9 hook test files) + `tools/tests/*.py` (5 tools test files) — 14 test files total verify hook + tool regex/logic changes don't introduce false-positives (which silently block legitimate work) or false-negatives (which silently let attacks through). Aggregate **316/316 passing — empirically verified 2026-05-07 cron F43** (growth trace: 215/234 (2026-05-06 evening, 13 files) → 305/305 (F38: +test-agent-output-scan SB-140 mitigation #2 + test-mindfulness 22→28 + test-malware-block 8→14 + 3 prior partial-failures closed: end-of-cycle-stamp-diff-suppression 21→22, mode-enforcement 0/0 collection→40/40, questions 33→51) → 316/316 (F40 test-mode-enforcement 40→47 cross-mode-composability + F41 test-agent-output-scan 19→23 SB-099 abdication phrases Phase 3)). Run via `/opt/devops-solutions-information-hub/.venv/bin/python /root/tools/run-tests.py` — the unified regression runner. Run before claiming any hook fix done (P4 — Declarations Aspirational Until Verified).
